...
while read -r URL; do
	...
	occurences=$(grep -E -o -i "\bstreet|fast food\b" ../dumps-text/$basename-$lineno.txt | wc -l)
	...
	contexte=$(grep -E -A2 -B2 "\bstreet|fast food\b" ../dumps-text/$basename-$lineno.txt > ../contextes/$basename-$lineno.txt)
	echo "$contexte"
